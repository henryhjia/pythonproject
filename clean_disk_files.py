    ...
    if CleanDiskFiles.JSON_KEY_TO_FIND in json_data:
      ...
      if time_diff > timedelta(days=CleanDiskFiles.DAYS_DELTA):
        for file_path in directory_path.glob(pattern):
          print(f'{index} {json_file.name} {pattern} {json_date_tz} {current_time} > {CleanDiskFiles.DAYS_DELTA} days, REMOVE')
          file_path.unlink()
      else:
        print(f'{index} {json_file.name} {pattern} {json_date_tz} {current_time} < {CleanDiskFiles.DAYS_DELTA} days, KEEP')
...
if __name__ == '__main__':
  ...
  parser.add_argument(
    'directory', type=Path, help='help text')
